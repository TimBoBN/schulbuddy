...
def statistics():
    ...
    try:
        ...
        stats = UserStatistic.query.filter(
            UserStatistic.user_id == current_user.id, UserStatistic.date >= start_date
        ).all()
            ...
            if subject_grades:
                ...
                subject_performance[subject] = {
                    ...
                    "trend": (
                        ...
                        and subject_grades[-1].grade < avg_grade
                        else "stable"
                    ),
                }
            ...
            if achievement.id in earned_achievement_ids:
                ...
                if achievement.condition_type == "level":
                    current_progress = min(
                        100, (current_user.level / achievement.condition_value) * 100
                    )
                elif achievement.condition_type == "points":
                    current_progress = min(
                        100,
                        (current_user.total_points / achievement.condition_value) * 100,
                    )
                elif achievement.condition_type == "streak":
                    current_progress = min(
                        100,
                        (current_user.current_streak / achievement.condition_value)
                        * 100,
                    )
                    ...
                    current_progress = min(
                        100, (completed_tasks / achievement.condition_value) * 100
                    )
...
def notifications():
    ...
    try:
        ...
        return render_template(
            "notifications.html", notifications=notifications, unread_count=unread_count
        )
    except Exception as e:
            ...
            tasks_data.append(
                {
                    ...
                    "Erledigt am": (
                        task.completed_date.strftime("%d.%m.%Y %H:%M")
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            overview_data = {
                ...
                "Wert": [
                    ...
                    len(grades),
                    (
                        f"{sum(g.grade for g in grades)/len(grades):.2f}"
                        if grades
                        else "Keine Noten"
                    ),
            ...
            if tasks_data:
                pd.DataFrame(tasks_data).to_excel(
                    writer, sheet_name="Aufgaben", index=False
                )

            # Noten-Sheet
            if grades_data:
                pd.DataFrame(grades_data).to_excel(
                    writer, sheet_name="Noten", index=False
                )
...
def export_pdf():
    ...
    try:
        ...
        content.append(
            Paragraph(f"Benutzer: {current_user.username}", styles["Normal"])
        )
        content.append(
            Paragraph(
                f"Export-Datum: {datetime.now().strftime('%d.%m.%Y um %H:%M Uhr')}",
                styles["Normal"],
            )
        )
        ...
        overdue_tasks = [
            t for t in open_tasks if t.due_date and t.due_date < datetime.now().date()
        ]
        ...
        if tasks:
            ...
            for task in sorted(
                tasks,
                key=lambda x: x.due_date or datetime(2099, 12, 31).date(),
                reverse=True,
            )[
                :20
            ]:  # Nur die neuesten 20
            ...
            tasks_table = Table(
                tasks_data,
                colWidths=[2.5 * inch, 1.2 * inch, 0.8 * inch, 0.8 * inch, 0.8 * inch],
            )
        ...
        if grades:
            ...
            for grade in sorted(grades, key=lambda x: x.timestamp, reverse=True)[
                :25
            ]:  # Nur die neuesten 25
                grade_emoji = (
                    "🟢"
            ...
            grades_table = Table(
                grades_data,
                colWidths=[2.2 * inch, 1.2 * inch, 0.8 * inch, 1 * inch, 0.9 * inch],
            )
...
def streaks():
    ...
    try:
        ...
        streak_goals = [
            ...
            {
                "days": 7,
                "title": "Woche geschafft!",
                "icon": "⚡",
                "reward": "25 Punkte",
            },
            {
                "days": 14,
                "title": "Zwei Wochen Disziplin",
                "icon": "💪",
                "reward": "50 Punkte",
            },
            {
                "days": 30,
                "title": "Ein Monat stark!",
                "icon": "🏆",
                "reward": "100 Punkte",
            },
        ...
        for i in range(12):  # Letzte 12 Wochen
            ...
            for j in range(7):
                ...
                if check_date <= end_date:
                    week_days.append(
                        {"date": check_date, "active": check_date in activity_days}
                    )

            weekly_streaks.insert(0, {"week_start": week_start, "days": week_days})
...
def calculate_streak_stats(activity_days, end_date):
    ...
    current_month_days = [
        d for d in activity_days if d >= month_start and d <= end_date
    ]
    ...
    last_month_days = [
        d for d in activity_days if d >= last_month_start and d <= last_month_end
    ]
