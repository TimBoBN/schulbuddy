...
def index():
    ...
    try:
        ...
        subject_summary = defaultdict(
            lambda: {"tasks": 0, "completed": 0, "avg_grade": 0}
        )
        ...
        for subject, grade_list in grades_by_subject.items():
            if grade_list:
                subject_summary[subject]["avg_grade"] = round(
                    sum(grade_list) / len(grade_list), 2
                )
                # Template erwartet 'average' statt 'avg_grade'
                subject_summary[subject]["average"] = subject_summary[subject][
                    "avg_grade"
                ]
        ...
        if certificate_grades:
            certificate_average = sum(g.grade for g in certificate_grades) / len(
                certificate_grades
            )
    ...
    debug_info = {
        ...
        "database_stats": {
            ...
            "grades": db.session.execute(
                db.text("SELECT COUNT(*) FROM grades")
            ).scalar(),
            "activity_logs": db.session.execute(
                db.text("SELECT COUNT(*) FROM activity_logs")
            ).scalar(),
        },
        "app_settings": {
            "app_name": AppSettings.get_setting("app_name", "SchulBuddy"),
            "session_timeout": AppSettings.get_setting(
                "session_timeout_minutes", "120"
            ),
            "max_file_size": AppSettings.get_setting("max_file_size", "16"),
            "enable_gamification": AppSettings.get_setting(
                "enable_gamification", "True"
            ),
            "enable_notifications": AppSettings.get_setting(
...
def check_session_timeout():
    ...
    if current_user.is_authenticated:
        ...
        warning_minutes = int(
            AppSettings.get_setting("auto_logout_warning_minutes", "5")
        )
        ...
        if last_activity:
            ...
            # Warnung vor Ablauf
                ...
                session["logout_warning"] = (
                    f"Automatischer Logout in {remaining_minutes:.0f} Minuten"
                )
