    ...
    return jsonify(
        {
            "api_key": api_key,
            "created_at": (
                current_user.api_key_created_at.isoformat()
    ...
    if task.file:
        try:
            file_full_path = os.path.join(
                Config.UPLOAD_FOLDER, os.path.basename(task.file)
            )
    ...
    try:
        ...
        if old_tasks:
                ...
                if task.filename:
                    try:
                        file_path = os.path.join(
                            current_app.config["UPLOAD_FOLDER"], task.filename
                        )
            ...
            if deleted_count > 0:
                ...
                flash(
                    f"Automatische Bereinigung: {deleted_count} alte Aufgaben wurden gelöscht (Speicher gespart).",
                    "success",
                )
    except Exception as e:
        db.session.rollback()
    ...
    for task in completed_tasks:
        if task.completed_date:
            ...
            tasks_by_month[month_key].append(
                {"task": task, "month_name": month_name, "month_display": month_name}
            )
...
def auto_cleanup_old_tasks():
    ...
    try:
        ...
        # Finde alle erledigten Aufgaben älter als 14 Tage
        old_tasks = Task.query.filter(
            Task.completed == True, Task.completed_date < cutoff_date
        ).all()
        ...
        if deleted_count > 0:
            db.session.commit()
            print(
                f"Auto-Cleanup: {deleted_count} alte Aufgaben gelöscht, {len(deleted_files)} Dateien entfernt"
            )
