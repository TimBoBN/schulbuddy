...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            auto_logout_warning = int(
                request.form.get("auto_logout_warning_minutes", "5")
            )

            # Validiere Bereiche
            if max_users < 1 or max_users > 10000:
                flash(
                    "Maximale Benutzeranzahl muss zwischen 1 und 10000 liegen", "error"
                )
                return redirect(url_for("admin.admin_settings"))

            if max_login_attempts < 1 or max_login_attempts > 50:
                flash(
                    "Maximale Login-Versuche müssen zwischen 1 und 50 liegen", "error"
                )
                return redirect(url_for("admin.admin_settings"))
            ...
            settings = {
                ...
                "allowed_file_types": request.form.get(
                ...
                "enable_notifications": request.form.get("enable_notifications")
                == "on",
            }
    ...
    settings = {
        ...
        "allowed_file_types": AppSettings.get_setting(
            "allowed_file_types", "txt,pdf,doc,docx,jpg,jpeg,png,gif"
        ),
        "session_timeout_minutes": AppSettings.get_setting(
            "session_timeout_minutes", "120"
        ),
        "auto_logout_warning_minutes": AppSettings.get_setting(
        ...
        "enable_registration": AppSettings.get_setting("enable_registration", "False")
        == "True",
