...
class User(UserMixin, db.Model):
    ...
    grades = db.relationship(
        "Grade", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    achievements = db.relationship(
        "UserAchievement", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    statistics = db.relationship(
        "UserStatistic", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def get_totp_uri(self):
        ...
        return pyotp.totp.TOTP(self.totp_secret).provisioning_uri(
            name=self.email, issuer_name="SchulBuddy"
        )
...
class Task(db.Model):
    ...
    )  # Wann wurde die Aufgabe erledigt
    task_type = db.Column(
        db.String(50), default="homework"
...
class Achievement(db.Model):
    ...
    )  # 'streak', 'tasks_completed', 'grade_average', etc.
...
class ActivityLog(db.Model):
    ...
    activity_type = db.Column(
        db.String(50), nullable=False
    )  # 'task_completed', 'grade_added', 'login', etc.
    activity_data = db.Column(
...
class Notification(db.Model):
    ...
    notification_type = db.Column(
        db.String(50), nullable=False
    )  # 'reminder', 'achievement', 'deadline', etc.
    ...
    try:
        ...
        for user in User.query.all():
            ...
            while current_date <= end_date:
                ...
                if not UserStatistic.query.filter_by(
    ...
    try:
        if Achievement.query.count() == 0:
            achievements = [
                ...
                Achievement(
                    name="Legende",
                    description="2000 Punkte gesammelt",
                    icon="🏅",
                    points=200,
                    condition_type="points",
...
class UserStatistic(db.Model):
    ...
    def update_daily_stats(user_id, date_obj=None):
        ...
        stat = UserStatistic.query.filter_by(user_id=user_id, date=date_obj).first()
